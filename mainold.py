...
numeric_features = ['budget_formazione', 'fatturato_conc(B)', 'finanziamenti', 'valore_startup','valore_mercato_totale', 'incremento_tec', 'media_fatturato', 'tasso_di_crescita_dip','numero_operatori', 'dip_ingegneri', 'dip_business', 'dip_vendite', 'dip_design','dip_informatica', 'dip_amministrativo', 'dip_controllo_qualità', 'dip_ricerca','dip_risorseumane', 'dip_assistenza'
]
target = 'stage'
predxg=1
predcat=0
...
if not df.empty:
    nomi=df[numeric_features].columns
    dati_standardizzati = scaler.fit_transform(df[numeric_features])
    dati_standardizzati= pd.DataFrame(dati_standardizzati,columns=nomi)
    df_risultato = pd.concat([dati_standardizzati,df[categorical_features]],axis=1)
    df_risultato.columns = df_risultato.columns.str.replace(' ', '')
    nan_count = df_risultato.isna().sum()
    print(df_risultato.columns)
    print(df_risultato)
# Eseguire il modello random forest    
    #predrand= random_forest_model(data, categorical_features, numeric_features, target, df_risultato)
# Eseguire il modello XGBoost
    #predxg = xgboost_model(data, categorical_features, numeric_features, target, df_risultato)
# Eseguire il modello CatBoost
    predcat = catboost(data, categorical_features, numeric_features, target, df_risultato)
    results_message = f"Predizione XGBoost: {stage[predxg]}\nPredizione CatBoost: {stage[predcat]}\n Predizione Random forest: {stage[predrand]}\n"
